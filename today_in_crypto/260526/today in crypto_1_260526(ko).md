...
이번 Safe 관련 사고의 핵심은 '지갑 자체가 털렸나?'보다 '지갑에 붙인 외부 모듈이 얼마나 큰 권한을 갖고 있었나?'이다. Safe는 여러 사람이 함께 서명해야 자산이 움직이는 멀티시그 지갑으로 많이 쓰인다. DAO나 프로젝트 금고가 자주 사용한다. 그런데 여기에 외부 모듈을 붙이면 특정 작업을 자동화할 수 있다. 이를테면 매월 누군가에게 운영비 얼마를 자동으로 보낼 수 있다. 이런 부분은 매번 멀티시그 3명이 승인하려면 시간이 오래 걸리고 비효율적일 거다. 그래서 Safe에 모듈을 붙여서 '이 모듈은 우리가 허락한 프로그램이니까 이 모듈이 정해진 규칙 안에서 요청하면 매번 3명의 서명을 받지 않아도 Safe 자금을 움직일 수 있게 해줘 아라쮜~?'라고 하는 것이다. 
